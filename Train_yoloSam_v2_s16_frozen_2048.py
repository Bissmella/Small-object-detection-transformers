...
from basics.test_samS16_2048 import test
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        for key in SAM_weights.keys():
            if key.startswith("image_encoder") and not key.startswith("image_encoder.patch_embed") and not key.startswith("image_encoder.pos_embed") and key.find("rel_pos") == -1: 
                print(key)
    ...
    if rank in [-1, 0]:
        ...
        testloader = create_dataloader(test_path, imgsz, batch_size, gs, opt,  # testloader     #*changed
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        #doing it once again:
        for name, param in model.named_parameters():
         if name.startswith("image_encoder") and not name.startswith("image_encoder.patch_embed") and not name.startswith("image_encoder.pos_embed") and not "rel_pos" in name:
             param.requires_grad = False
             print(name, param.requires_grad)
        ...
        for i, (imgs, irs, targets, paths, _) in pbar:  # batch zjq  -------------------------------------------------------------
            ...
            if down_factor>1:
                ...
                imgs=F.interpolate(imgs,size=[i*down_factor for i in imgs.size()[2:]], mode='bilinear', align_corners=True)
                #irs = F.interpolate(ir_image,size=[i*4 for i in ir_image.size()[2:]], mode='bilinear', align_corners=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str,default='codes/models/SRyolo_SAM_v3_orig_2048.yaml', help='model.yaml path') #yolov5s
    ...
    parser.add_argument('--project', default='outputs_SAM/yoloSAM_v2_s16_frozen_RGBIR_2048/run/train', help='save to project/name')
    ...
    # Set DDP variables
    opt.world_size = int(os.environ['WORLD_SIZE']) if 'WORLD_SIZE' in os.environ else 1 #* changed int(os.environ['WORLD_SIZE']) if 'WORLD_SIZE' in os.environ else 1
    opt.global_rank = int(os.environ['LOCAL_RANK']) if 'RANK' in os.environ else -1#int(os.environ['LOCAL_RANK']) if 'RANK' in os.environ else -1
