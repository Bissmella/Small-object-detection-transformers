...
def date_modified(path):       #*changed   =__file__
    # return human-readable file modification date, i.e. '2021-3-26'
    t = datetime.datetime.fromtimestamp(Path(path).stat().st_mtime)
    return f'{t.year}-{t.month}-{t.day}'


def git_describe(path):  # path must be a directory       #* changed    =Path(__file__).parent
    # return human-readable git description, i.e. v5.0-5-g3e25f1e https://git-scm.com/docs/git-describe
    path = Path(path).parent                          #*newly added
...
def select_device(device='', batch_size=None):
    ...
    s = f'YOLOv5 🚀  torch {torch.__version__} '  # string  #*removed {git_describe() or date_modified()}
...
def parse_model(d, string, ch,config):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d_):  # from, number, module, args
        ...
        if m in [Conv, ACmix, Bottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, BottleneckCSP2, SPPCSP, C3, AttentionModel]:
            ...
            args.append([ch[x + 1] for x in f])         #x + 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ch', type=int,default=4, help = '3 4 16 midfusion1:64 midfusion2,3:128 midfusion4:256')    ##64
    ...
    parser.add_argument('--project', default='outputs/pix_fusion', help='save to project/name')
