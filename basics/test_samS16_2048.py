...
def test(data,
    ...
    for batch_i, (img, ir, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)): #zjq
        ...
        ir=F.interpolate(ir,size=[i//2 for i in ir.size()[2:]], mode='bilinear', align_corners=True)
        img=F.interpolate(img,size=[i*2 for i in img.size()[2:]], mode='bilinear', align_corners=True)  #* added for SAM backbone
        img=F.interpolate(img,size=[i*2 for i in img.size()[2:]], mode='bilinear', align_corners=True)
        ir=F.interpolate(ir,size=[i*4 for i in ir.size()[2:]], mode='bilinear', align_corners=True)
        pixel_mean= [123.675, 116.28, 103.53]
        pixel_std= [58.395, 57.12, 57.375]
        img = img.permute(0, 2, 3, 1)
        img = (img - torch.tensor(pixel_mean).to(device)) / torch.tensor(pixel_std).to(device)
