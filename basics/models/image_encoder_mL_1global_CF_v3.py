...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        for i in range(6):
            ...
            self.iblocks.append(blk)

        self.fc_layer = MLPBlock(embedding_dim=96, mlp_dim=192, act=nn.GELU)
        #nn.Linear(192, 192) #removed (img_size //16) *
        #local blocks
        self.blocks = nn.ModuleList()
        for i in range(6):  #before depth
            block = Block(
                dim=96,    #before embed dim
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                norm_layer=norm_layer,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        r = self.channel_embed_r(r)      #x1[:,0,:,:].unsqueeze(1)).view(bs, 1, 1, 1536)
        g = self.channel_embed_r(g)      #x1[:,1,:,:].unsqueeze(1)).view(bs, 1, 1, 1536)
        b = self.channel_embed_r(b)      #x1[:,2,:,:].unsqueeze(1)).view(bs, 1, 1, 1536)
        i = self.channel_embed_r(i)       #x1[:,3,:,:].unsqueeze(1)).view(bs, 1, 1, 1536)
        
        if self.pos_embed is not None:
            if r.shape[1] == self.pos_embed.shape[1]:
                r = r + self.pos_embed
                g = g + self.pos_embed
                b = b + self.pos_embed
                i = i + self.pos_embed
        ...
        if self.pos_embed2 is not None:
            if x.shape[1] == self.pos_embed2.shape[1]:
                x = x + self.pos_embed2
...
def channel_unpartition(x, h, w, c):
    B = x.shape[0] // (h * w)
    x = x.view(B, h, w, c, -1)
    return x
