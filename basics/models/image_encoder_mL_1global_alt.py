...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        window_size = [4, 3, 4, 3, 4, 3, 4, 3, 4, 3, 3]
        padding = [False, True, False, False, False, True, False, False, False, True, False]
        for i in range(depth):
            block = Block(
                ...
                window_size= 4 if i not in global_attn_indexes else 0,  #window_size[i]
                top_padding = False,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y[2] =  F.interpolate(self.neck3(y[2].permute(0, 3, 1, 2)), scale_factor=4, mode='bilinear', align_corners=False)
        return y
...
def window_partition(x: torch.Tensor, window_size: int, top_padding: bool) -> Tuple[torch.Tensor, Tuple[int, int]]:
    """
...
def window_unpartition(windows: torch.Tensor, window_size: int, pad_hw: Tuple[int, int], hw: Tuple[int, int], top_padding: bool) -> torch.Tensor:
    """
