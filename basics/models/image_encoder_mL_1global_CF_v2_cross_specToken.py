...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.spec_tokens = nn.Parameter(torch.randn(1, 1, 16, 192))  ##192 is embed dimension
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #TODO: 4 layers fo block attention
        for i in range(4):
            x = self.blocks[i](x)
            if i in (9, 10):
                y.append(x)
...
def window_unpartition(
    ...
    Hp, Wp = pad_hw
    H, W = hw
    B = windows.shape[0] // (Hp * Wp // wh // ww)
    x = windows.view(B, Hp // wh, Wp // ww, wh, ww, -1)
