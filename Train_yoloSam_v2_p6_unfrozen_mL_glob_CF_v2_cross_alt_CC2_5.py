...
def train(hyp, opt, device, tb_writer=None):
    ...
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, opt,      #*changed
                                        ...
                                        workers=opt.workers,
                                        image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '), fold = 'labels05')
    # else:
    ...
    if rank in [-1, 0]:
        ...
        testloader = create_dataloader(test_path, imgsz_test, batch_size, gs, opt,  # testloader     #*changed  batch_size//2
                                    ...
                                    prefix=colorstr('val: '), fold = 'labels05')[0]
        # else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str,default='codes/models/SRvedai5.yaml', help='data.yaml path')
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ckpt = opt.resume if isinstance(opt.resume, str) else get_latest_run('/home/bbahaduri/sryolo/outputs_SAM/yoloSAM_v2_p6_multiL_RGBIR_glob_CF_v2_ViT1/run/train/exp/')  # specified or most recent path
